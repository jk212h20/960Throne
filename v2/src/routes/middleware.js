...
function requireRelay(req, res, next) { const provided = req.headers['x-relay-secret'] || req.body?.relaySecret || req.query?.relaySecret; if (config.dgtRelaySecret && provided === config.dgtRelaySecret) return next(); return res.status(401).json({ error: 'Invalid relay secret' }); }
