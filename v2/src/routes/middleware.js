...
function validAdminToken(token) { if (!token || !token.includes('.')) return false; const [raw, sig] = token.split('.'); return crypto.timingSafeEqual(Buffer.from(sig), Buffer.from(sign(raw))); }
...
module.exports = { makeAdminToken, requireAdmin, requirePlayer, requireRelay, parseCookies };
