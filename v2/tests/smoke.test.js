...
(async () => {
  ...
  const lightningNode = require('../src/domain/lightningNode');
  assert.equal(lightningNode.normalizeLightningAddress('LIGHTNING:alice@example.com'), 'alice@example.com');
  assert.throws(() => lightningNode.normalizeLightningAddress('not-an-address'), /Invalid Lightning Address/);
  ...
  const manualPair = await engine.adminSetPairing({ kingId: a.id, challengerId: b.id, position: 518 });
  assert.equal(manualPair.success, true, 'admin should be able to manually set king/challenger pairing');
  s = engine.getState();
  assert.equal(s.king.name, 'Alice');
  assert.equal(s.game.challenger_name, 'Bob');
  assert.equal(s.game.chess960_position, 518);
  dgt.setExpectedPosition(518);
  dgtState = dgt.update({ fen: chess960.positionToStartingFen(518), clock: { white: 180, black: 180, running: false, activeSide: null } });
  assert.equal(engine.maybeAutoStartFromDgt(dgtState).started, false, 'setup alone should not start before clock runs');
  const afterWhiteMoveFen = 'rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR';
  dgtState = dgt.update({ fen: afterWhiteMoveFen, clock: { white: 176, black: 180, running: true, activeSide: 'black' } });
  assert.equal(dgtState.setupOk, false, 'moved board is no longer exact setup');
  const lateStart = engine.maybeAutoStartFromDgt(dgtState);
  assert.equal(lateStart.started, true, 'DGT should late-start after confirmed setup when play has clearly begun');
  assert.equal(lateStart.fallback, true, 'late-start should be marked as fallback');
  assert(engine.getState().game.table_started_at, 'late-start fallback should mark table started');
  assert.equal(engine.finalizeGame(engine.getState().game.id, 'draw').success, true);
  const reportPair = await engine.adminSetPairing({ kingId: a.id, challengerId: b.id, position: 518 });
  assert.equal(reportPair.success, true);
  const reportGameId = engine.getState().game.id;
  assert.equal(engine.reportResult(a.id, 'king_won').success, true, 'result report should auto-start and finalize an unstarted game');
  const reportedGame = db.getGame(reportGameId);
  assert(reportedGame.table_started_at, 'result report should rescue missing table start');
  assert.equal(reportedGame.result, 'king_won');
  db.addSats(b.id, 100);
